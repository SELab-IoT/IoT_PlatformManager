...
object XACMLParser{
  ...
  def parseAll(xml: Elem): SyntaxTree = {
    ...
    else throw ParseException("Not PolicySet or Policy") //Handle Exception
  }

  /** Parse 1 PolicySet Tag **/
  def parsePolicySet(policySet: Elem):PSTree = {
    val algID = PolicyCombiningAlgorithmParser.AlgIdAttribute
    val policyCombAlg:CombiningAlgorithm  =
      policySet.attribute(algID) match {
        case Some(nodeSeq) => nodeSeq.lastOption.get.text // getOrElse 사용한 예외 처리는 필요 없을 듯
        case None => throw ParseException("No PolicyCombiningAlg")//Handling No PolicyCombining Algorithm : 디폴트 적용하거나 예외 던지거나.
      }
  ...
  /** Parse Policies **/
  def parsePolicyList(policies: NodeSeq, policyCombAlg: CombiningAlgorithm): Combine[PTree] = {
  ...
  def parsePolicy(policy: Node): PTree = {
    val algID = RuleCombiningAlgorithmParser.AlgIdAttribute
    val ruleCombAlg = policy.attribute(algID) match {
      ...
      case None => throw ParseException("No RuleCombiningAlg")//Handling No PolicyCombining Algorithm : 디폴트 적용하거나 예외 던지거나.
    }
    ...
    target match {
      case Some(t) =>
        val anyOfList =
          ...
          else (t \ "AnyOf").map(parseAnyOf).reduce(And)
        Target(anyOfList)
